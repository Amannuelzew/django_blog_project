...
def index(request):
    latest_articles = sorted(articles, key=lambda x: x["date"], reverse=True)
    return render(request, "blog/index.html", {"articles": latest_articles[-3:]})


def posts(request):
    return render(request, "blog/posts.html", {"articles": articles})


def post_detail(request, slug):
    post = next(x for x in articles if x["slug"] == slug)
